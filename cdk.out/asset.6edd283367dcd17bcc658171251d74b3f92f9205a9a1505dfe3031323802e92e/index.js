...
async function getSwapiMerged(event, ddbClient2, context) {
  ...
  if (!id) {
    console.log("Missing id query parameter");
    return {
      statusCode: 400,
  ...
  if (cachedItem.Item) {
    const ttl = (0, import_util_dynamodb.unmarshall)(cachedItem.Item).ttl;
  ...
  try {
    ...
    if (!this.isCache) {
      const CharacterCache = {
        ...
        //lastmission: mission,
        lastmission: "Climb a glacier",
        ttl: Math.floor(Date.now() / 1e3) + 60,
        createdAt: (/* @__PURE__ */ new Date()).toISOString()
      };
    ...
    const result = await ddbClient2.send(
      new import_client_dynamodb.PutItemCommand({
        ...
        Item: (0, import_util_dynamodb.marshall)(Character)
      })
    );
    if (this.isCache) {
      return {
        ...
        body: JSON.stringify({
          ...
          item: (0, import_util_dynamodb.unmarshall)(cachedItem.Item)
        })
      };
    } else {
      return {
        statusCode: 200,
        body: JSON.stringify({ message: "Character stored", Character })
      };
    }
  } catch (err) {
