...
// src/services/swapi_merged/handler.ts
var handler_exports = {};
...
// src/services/swapi_merged/GetSwapiMerged.ts
var https2 = __toESM(require("https"));

// src/services/swapi_merged/GetLastMission.ts
var getLastMission = async () => {
  try {
...
async function getSwapiMerged(event, ddbClient2, context) {
  ...
  const cachedItem = await ddbClient2.send(
  ...
  if (cachedItem.Item) {
    ...
    const unmarshalled = (0, import_util_dynamodb.unmarshall)(cachedItem.Item);
  ...
  try {
    const response = isCache ? { data: cachedData } : await axiosInstance.get(`${baseUrl}${id}/`);
    console.log("Response data:", response.data);
    const mission = isCache ? { data: cachedData } : await getLastMission();
    ...
    const result = await ddbClient2.send(
      new import_client_dynamodb.PutItemCommand({
        ...
        Item: (0, import_util_dynamodb.marshall)(characterData)
      })
    );
    if (!isCache) {
      const CharacterCache = {
        ...
        ttl: Math.floor(Date.now() / 1e3) + 1800,
        createdAt: (/* @__PURE__ */ new Date()).toISOString()
      };
      await ddbClient2.send(
        new import_client_dynamodb.PutItemCommand({
          TableName: process.env.TABLE_CACHE_NAME,
          Item: (0, import_util_dynamodb.marshall)(CharacterCache)
        })
      );
    }
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: isCache ? "Returned from cache (but wrote to DB)" : "Fetched and stored",
        item: characterData
      })
    };
  } catch (err) {
